...
def detect_payout_discrepancies(company, date_from=None, date_to=None):
    ...
    """
    try:
        from shopify_connector.reconciliation import reconcile_payout
    ...
    for payout in payouts:
        ...
        if recon.status == "discrepancy":
            ...
            exc = _create_exception(
                ...
                description=(
                    f"Shopify payout {payout.shopify_payout_id} has a net variance of "
                ...
                reference_label=f"Shopify payout {payout.shopify_payout_id}",
                details={
        ...
        # Also flag payouts with unmatched transactions as fee variances
        if recon.fee_variance != 0 and recon.status != "discrepancy":
            exc = _create_exception(
                ...
                title=f"Fee variance: {recon.fee_variance} on payout {payout.shopify_payout_id}",
                description=(
                    f"Shopify payout {payout.shopify_payout_id} has a fee variance of "
