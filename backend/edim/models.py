...
class SourceSystem(models.Model):
    ...
    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name="edim_source_systems")
    public_id = models.UUIDField(default=uuid.uuid4, editable=False, unique=True)

    code = models.CharField(max_length=50)
    name = models.CharField(max_length=255)
    system_type = models.CharField(max_length=20, choices=SystemType.choices)
    trust_level = models.CharField(max_length=20, choices=TrustLevel.choices, default=TrustLevel.INFORMATIONAL)
    ...
    def save(self, *args, **kwargs):
        if not write_context_allowed({"command", "migration", "bootstrap"}) and not getattr(settings, "TESTING", False):
            raise RuntimeError("SourceSystem is an EDIM write model. Use edim.commands to modify.")
...
class MappingProfile(models.Model):
    ...
    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name="edim_mapping_profiles")
    public_id = models.UUIDField(default=uuid.uuid4, editable=False, unique=True)

    source_system = models.ForeignKey(SourceSystem, on_delete=models.CASCADE, related_name="mapping_profiles")
    ...
    def save(self, *args, **kwargs):
        if not write_context_allowed({"command", "migration", "bootstrap"}) and not getattr(settings, "TESTING", False):
            raise RuntimeError("MappingProfile is an EDIM write model. Use edim.commands to modify.")
        super().save(*args, **kwargs)
...
class IdentityCrosswalk(models.Model):
    ...
    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name="edim_crosswalks")
    public_id = models.UUIDField(default=uuid.uuid4, editable=False, unique=True)

    source_system = models.ForeignKey(SourceSystem, on_delete=models.CASCADE, related_name="crosswalks")
    ...
    nxentra_id = models.CharField(max_length=255, blank=True, default="", help_text="public_id of the Nxentra entity")
    nxentra_label = models.CharField(
        max_length=500, blank=True, default="", help_text="Human-readable label of Nxentra entity"
    )

    status = models.CharField(max_length=20, choices=CrosswalkStatus.choices, default=CrosswalkStatus.PROPOSED)
    ...
    def save(self, *args, **kwargs):
        if not write_context_allowed({"command", "migration", "bootstrap"}) and not getattr(settings, "TESTING", False):
            raise RuntimeError("IdentityCrosswalk is an EDIM write model. Use edim.commands to modify.")
        super().save(*args, **kwargs)
...
class IngestionBatch(models.Model):
    ...
    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name="edim_batches")
    public_id = models.UUIDField(default=uuid.uuid4, editable=False, unique=True)
    ...
    file_checksum = models.CharField(max_length=64, blank=True, default="", help_text="SHA-256 of uploaded file")
    ...
    def save(self, *args, **kwargs):
        if not write_context_allowed({"command", "migration", "bootstrap"}) and not getattr(settings, "TESTING", False):
            raise RuntimeError("IngestionBatch is an EDIM write model. Use edim.commands to modify.")
        super().save(*args, **kwargs)
...
class StagedRecord(models.Model):
    ...
    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name="edim_staged_records")
    ...
    mapped_payload = models.JSONField(null=True, blank=True, help_text="Canonical payload after mapping rules applied")
    mapping_errors = models.JSONField(default=list, blank=True, help_text="List of mapping error messages")

    # Validation results (populated during Validate step)
    validation_errors = models.JSONField(default=list, blank=True, help_text="List of validation error messages")
    is_valid = models.BooleanField(null=True, blank=True, help_text="True if record passed validation")

    # Crosswalk resolution results
    resolved_accounts = models.JSONField(default=dict, blank=True, help_text="Map of external_id -> nxentra_id")
    ...
    def save(self, *args, **kwargs):
        if not write_context_allowed({"command", "migration", "bootstrap"}) and not getattr(settings, "TESTING", False):
            raise RuntimeError("StagedRecord is an EDIM write model. Use edim.commands to modify.")
        # Enforce immutability of raw_payload and row_hash after creation
        if not self._state.adding and self.pk:
            original = StagedRecord.objects.filter(pk=self.pk).values("row_hash").first()
