...
def _raise_http_error(status_code: int):
    """Return a callable that mocks requests.get to raise HTTPError(status_code)."""
...
def test_sync_payouts_403_returns_unavailable_not_failure(active_store):
    ...
    with patch("shopify_connector.commands.requests.get", side_effect=_raise_http_error(403)):
...
def test_sync_payouts_5xx_still_fails(active_store):
    ...
    with patch("shopify_connector.commands.requests.get", side_effect=_raise_http_error(503)):
        result = commands.sync_payouts(active_store)
...
def test_sync_orders_403_returns_unavailable_status(active_store):
    ...
    that.
    """
    with patch("shopify_connector.tasks.requests.get", side_effect=_raise_http_error(403)):
...
def test_sync_orders_5xx_still_returns_error_status(active_store):
    with patch("shopify_connector.tasks.requests.get", side_effect=_raise_http_error(503)):
        result = tasks._sync_orders(
            active_store,
            "2026-05-25T00:00:00+00:00",
            "2026-06-01T00:00:00+00:00",
