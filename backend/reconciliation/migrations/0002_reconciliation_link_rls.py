"""Enable Postgres row-level security on reconciliation_link.

P5 follow-up (adversarial review): ReconciliationLink is a mutable per-company
financial read model, so it joins the same RLS tenant-isolation regime as its
peers (accounting_account / accounting_journalentry / accounting_journalline,
see accounts/migrations/0008, 0010, 0014). Without RLS, tenant isolation would
rely solely on application-level company filtering. RLS is Postgres-only; this
migration is a no-op on SQLite (the test backend).
"""
