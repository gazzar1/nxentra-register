...
"""Reconciliation command surface — A86.8 (2026-05-26).

Moved from `accounting/bank_reconciliation.py`. Houses the
operator-initiated commands that mutate reconciliation state by
emitting `ReconciliationMatch*` events:

- `auto_match_statement` — heuristic auto-match across settlement
  pre-pass, platform pre-pass, and generic GL match.
- `manual_match` — operator picks a (bank_line, journal_line) pair.
- `unmatch_line` — reverse a prior match (A19: reverses synthesized
  clearance JEs).
- `exclude_line` — same as unmatch but ends in EXCLUDED.
- `resolve_difference` — A16 difference resolution; posts the
  adjustment JE that drains the EBD residual.

Plus the corresponding previews:

- `preview_auto_match` — dry-run of the settlement pre-pass (the only
  step that synthesizes JEs).
- `preview_unmatch_line` — dry-run showing which JEs would reverse.

Helpers in this module are private (underscore-prefixed) and only
needed by the commands above. The pure planner
(`_plan_settlement_prepass_matches`) and confidence helpers live in
`reconciliation/matching.py`; nothing here imports from
`accounting/bank_reconciliation.py` other than for the import-only
side of statement-import (which stays in `accounting`).

A86.7b context: every command in this module runs the
ReconciliationProjection synchronously inside its `@transaction.atomic`
so canonical match state on BankStatementLine is up to date by the
time the command returns. The legacy direct-mutation paths are gone.

Backward-compat: `accounting/bank_reconciliation.py` re-exports
everything in this module so existing test imports keep working.
"""
...
# a period override. Mirrors the constant in settlement_imports.py.
...
def _run_reconciliation_projection_sync(company) -> None:
    ...
    canonical state is up to date when the command returns. The
    ...
    """
    # Local import: reconciliation.projections imports from accounting,
    # so a top-level import would be circular-prone in some boot orders.
...
def _emit_match_unmatched(
    *,
    ...
    this and clears the canonical match fields on the bank line (or
    sets match_status to EXCLUDED when final_status='EXCLUDED').
...
def _emit_match_confirmed(
    *,
    ...
    A86.7b (2026-05-26): the projection is the sole writer of canonical
    bank-line match state. This event is the only mechanism for that
    write — no legacy direct-mutation path exists.
...
# =============================================================================
# Settlement clearance JE creation (helper for the settlement pre-pass)
# =============================================================================
...
# =============================================================================
# Pre-pass execute paths (platform + settlement) — emit Confirmed events
# =============================================================================
...
def _settlement_prepass_match(
    ...
    *,
    ...
    are delegated to `_plan_settlement_prepass_matches` (in
    `reconciliation/matching.py`); this function only applies the plan —
    creates clearance JEs (honoring an optional period override) and
    updates read-model state.
    ...
    with command_writes_allowed():
        for bank_line in unmatched_bank_lines:
            ...
            if best_match and best_confidence >= AUTO_MATCH_THRESHOLD:
                ...
                # same way nearby code does for JournalEntry.
...
# =============================================================================
# manual_match / unmatch_line / exclude_line
# =============================================================================
...
def preview_unmatch_line(
    ...
    # Identify the clearance JE (if any).
    clearance_je = None
    if journal_line and journal_line.entry.source_module == "payment_settlement_clearance":
        clearance_je = journal_line.entry
...
def unmatch_line(
    ...
    the reversed clearance JE list — projection clears the canonical
    match fields on the bank line.
    """
    require(actor, "accounting.reconciliation")
...
def exclude_line(
    ...
    A86.5 (2026-05-26): also emits ReconciliationMatchUnmatched with
    final_status="EXCLUDED" — projection sets match_status to EXCLUDED.
