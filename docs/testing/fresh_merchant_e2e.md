...
| 2 | Negative checks: try a database name with a space / >10 chars; short password; unchecked ToS | Inline client-side errors: "Use a single word with no spaces" / "Maximum 10 characters" / "Password does not meet all the requirements below" (rules checklist: 8+ chars, uppercase, number, special) / ToS error. No API call fires |
