...
| Known gap | Consolidating the dual Shopify webhook stacks remains future work. Posted-journal validity is centrally enforced for `ISOLATED_SHADOW_LEDGER_V1` by A3 at both the emit and apply boundaries. Current gate status lives in the [live tracker](../status/constrained_pilot_status.md). |
...
| Known gap | Same dual-webhook-stack consolidation debt as Shopify orders. Posted-journal enforcement itself is complete under A3 for the constrained pilot. |
...
| Known gap | Settlement-equation validation and posted-journal validation are separate financial invariants. Posted-journal validity is centrally enforced under A3 at both emit and apply; any remaining settlement-equation debt is separate from A3. Current gate status lives in the [live tracker](../status/constrained_pilot_status.md). |
...
| Known gap | "Open versus cleared" remains derived from journal state and reconciliation links; there is no standalone EBD lifecycle model. A5's durable-visible-outcome gate is **COMPLETE** for `ISOLATED_SHADOW_LEDGER_V1` under the [2026-08-30 closure review](../audits/2026-08-30-a5-final-closure-review.md). Current gate status lives in the [live tracker](../status/constrained_pilot_status.md). |
...
| Legacy / competing | No competing posted-journal model exists. Earlier model, policy and command checks (emitter payload validation, model-level postability checks, period gates in `accounting/policies.py` / `accounting/validation.py`) remain as preconditions or defense-in-depth checks, but the canonical posted-journal acceptance implementation is [backend/accounting/journal_invariant.py](../../backend/accounting/journal_invariant.py), enforced at the serialized emit boundary and the single apply choke point. |
...
| Known gap | **A3 closed at both boundaries (PR1 core+scanner → PR2 emit → PR2b account-state serialization, merged 2026-08-09 → PR3 apply/replay).** The canonical invariant core is [backend/accounting/journal_invariant.py](../../backend/accounting/journal_invariant.py) (14 stable violation codes; read-only corpus scanner `audit_posted_journal_corpus`). **Emit:** every JOURNAL_ENTRY_POSTED emission goes through the serialized boundary `emit_posted_journal` ([backend/accounting/posted_journal_boundary.py](../../backend/accounting/posted_journal_boundary.py)) — exact prepared-payload emission, zero tolerance, account facts serialized against the CompanyEventCounter linearization point. **Apply (A3-PR3):** every event traverses the ONE apply choke point (`BaseProjection.process_pending` → [backend/projections/apply_validation.py](../../backend/projections/apply_validation.py); event types with registered apply validators — the journal family, owned by [backend/accounting/posted_journal_apply.py](../../backend/accounting/posted_journal_apply.py) — are validated there, and unregistered event types pass through apply validation unchanged) before any projection handler runs — covering the live drain, Celery, CLI, every rebuild entry point, and tenant replay; violations quarantine via `ProjectionTerminalSkip` (visible `ProjectionFailureLog`, stream advances — universal strict, no cutover state, founder decisions 2026-08-21). In-app restore verifies the restored posted-JE corpus fail-closed inside the restore transaction; the sibling doors (`journal_entry.reversed` / `.deleted` shape guards, chunked-family consume-to-quarantine) carry boundary-local guards. Because apply-mode checks are a strict subset of emit-mode checks, an event emitted through the boundary can never fail apply — quarantine only fires for pre-enforcement history, foreign streams, or payloads corrupted at rest; the precondition (apply consults the same account facts emit saw) is enforced by freezing both memo-classification inputs once an account carries posted history (`ledger_domain` via the frozen update-field set; `account_type` via `can_change_account_type`'s durable-evidence guard), and read-model lag on account resolution defers rather than quarantines. The event-fold report views and the AccountBalance verification family share the boundary's verdict too: account-derived memo classification, and folds skip any event whose persisted apply disposition is disqualifying (`excluded_posted_event_ids` / `posted_event_accepted_for_apply`), so reports and verification agree with the strict-quarantine balances. Remaining deferred: nine pre-existing lock-order pairs (Lock-Order Normalization workstream, availability hazards). |
...
| Inventory / COGS | Option B: item creation forced NON_STOCK; INVENTORY/COGS module mappings never seeded for pilots; `ShopifyStore.default_inventory_account` / `default_cogs_account` are **never written by any code path — legacy/dead configuration**, flagged by preflight if ever populated. Re-enabling inventory accounting is gated on the **Inventory Books-Delta Foundation** ([constrained_pilot_status.md](../status/constrained_pilot_status.md) § Open architectural decisions) — a provider-neutral plan/apply boundary making the old-to-new stored-books delta the canonical books movement across costing, purchases, voids and transfers. Interim posture: inventory emitters that cannot produce an exact current-ledger payload **fail loudly at the canonical emit boundary** rather than emitting unvalidated financial truth. |
