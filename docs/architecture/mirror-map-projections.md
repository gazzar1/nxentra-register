...
| Posted journals go through the canonical emit boundary; emit restores RLS context (PR #145) | `events/emitter.py` — `emit_posted_journal` / `prepare_posted_journal_for_emit`; `accounts/rls` restore rule | Architecture constitution rule 5; arch ratchets on posted emitters | The emitter's projection fallback is a separate concern from emission. |
