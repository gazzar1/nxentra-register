...
| Posted journals go through the canonical emit boundary; emit restores RLS context (PR #145) | `accounting/posted_journal_boundary.py` — `emit_posted_journal`; `accounting/journal_invariant.py` — `prepare_posted_journal_for_emit`; `accounts/rls.py` — `snapshot_rls_context` / `restore_rls_context` (the one restore rule, called from `events/emitter.py`) | Architecture constitution rule 5; arch ratchets on posted emitters | `events/emitter.py` is the generic event door and the caller of the restore rule, not the owner of the posted-journal boundary; its projection fallback is a separate concern from emission. |
