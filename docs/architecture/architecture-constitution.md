...
|---|---|
| Rule 1 — canonical path | **Materially enforced for `ISOLATED_SHADOW_LEDGER_V1`.** The [money spine](canonical-money-spine.md) names writers, readers, and legacy paths; central posted-journal validation is complete under A3 and durable terminal outcomes are complete under A5. Neither gate is deployed. Current gate status lives in the [live tracker](../status/constrained_pilot_status.md). |
...
| Rule 3 — one invariant implementation | **Central posted-journal enforcement is complete under A3** at both the emit and apply choke points, completed by A3-PR3 / PR #125. Some invariants are already canonical (settlement equation guard, pilot currency acceptance in `accounts/pilot_policy.py`); other invariant families may still contain duplication and remain documented architecture debt. Current gate status lives in the [live tracker](../status/constrained_pilot_status.md). |
...
| Rule 5 — versioned contracts | **Materially enforced for `ISOLATED_SHADOW_LEDGER_V1` by A4** (merged PR #107): runtime gates at shared boundaries, transactional audited activation, exhaustive setup/go-live preflight, ~80 acceptance tests. Current gate status and the subsequent re-closure history, including PR #137, live in the [constrained-pilot tracker](../status/constrained_pilot_status.md). |
