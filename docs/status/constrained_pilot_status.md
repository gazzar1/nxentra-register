...
| **A3** — one central posted-JE invariant at emit + apply | **Open — emit boundary enforced; apply/replay boundary remains open (A3-PR2 in review)** | Canonical invariant core + read-only corpus scanner merged (A3-PR1). A3-PR2 wires `require_valid_posted_journal()` in front of every JOURNAL_ENTRY_POSTED emitter (10 emit paths + external ingest) with zero tolerance for NEW events; the ±0.05 receipt/payment acceptance band is removed. **Historical events, apply/replay and rebuild remain unprotected** until A3-PR3, which is gated on a production corpus scan or an explicit compatibility decision. |
