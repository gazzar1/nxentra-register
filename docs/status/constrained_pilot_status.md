...
| **A3** — one central posted-JE invariant at emit + apply | **Open — core + scanner in review (A3-PR1)** | Canonical invariant module (`accounting/journal_invariant.py`, 14 stable codes) + read-only corpus scanner (`audit_posted_journal_corpus`) in review. **No runtime protection yet**: emit-boundary (PR2) and apply-boundary (PR3) enforcement pending. |
