...
- **Current precision policy (constrained pilot).** High-precision quantity and unit-cost evidence is preserved (6dp costs, exact valuation inputs); inventory and GL STORED books values follow the current two-decimal EGP/schema contract via the canonical `quantize_current_ledger_amount` (per-movement quantize, ROUND_HALF_EVEN). A stock quantity movement whose books value is 0.00 is still recorded — it moves quantity and writes stock-ledger evidence but creates no zero-value journal entry and consumes no journal identity. This is the pilot contract, not the final global policy.
