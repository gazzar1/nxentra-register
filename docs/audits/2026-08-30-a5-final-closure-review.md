...
| Paymob/Bosta settlement import (valid; blank batch ID; malformed numerics; imbalance/quarantine; ORPHAN_ORDER_ID; duplicate retry; non-EGP whole-file refusal) | 7 | 6 PASS + R1 | Per-row REJECTED evidence outside the batch atomics; the zero-substitution masquerade is pinned dead; orphan QUARANTINE flags exist iff the JE committed. The first six listed cases PASS; R1 is the non-EGP whole-file refusal envelope; `test_a5_pr3a`, `test_a5_pr3bc` |
