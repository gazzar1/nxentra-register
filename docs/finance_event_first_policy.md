...
> **The list above is illustrative, not the writer registry — the [canonical money spine](architecture/canonical-money-spine.md) and the live code are authoritative on who writes each fact.** Several models named above are **command-owned in the live code today**, not purely event-projected: e.g. `SalesInvoice` (created by `sales/commands.py`; classification tracked as open **A110** in `NEXT_TASKS.md`) and `BankStatement` / `BankStatementLine` (created by `import_bank_statement` — the money spine documents this as a **Rule 1 hybrid**: a direct write with no ingestion event, recorded as a known gap / tracked debt, **not** an ADR-granted exception). For the authoritative writer of any specific fact, consult the money spine (the writer registry) and the code — not this list; where they disagree, **the code/spine win**, and you must not refactor an existing command-owned model to match this list without a deliberate decision (ADR / roadmap item). The event-first *rule* (emit an event for new accounting movements; never patch a read model to fix a disagreement) still binds new work.
