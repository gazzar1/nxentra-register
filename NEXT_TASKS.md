...
- **P4 batch (remaining):** F2 (single-company login shows company chooser — DEFERRED: touches the auth login flow + a subtle AuthContext/login.tsx response-handling interaction; needs a careful trace, not a rushed polish), F5 (legacy `/shopify/reconciliation` MM/DD native date inputs — low value; that page is slated for retirement per F4), F14 (COD-awaiting-collection exposure tile — a feature/enhancement, not polish).
