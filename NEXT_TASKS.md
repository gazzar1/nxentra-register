...
- **P4 batch (remaining):** F2 (single-company login shows company chooser), F5 (legacy payout page MM/DD native date inputs), F6 (Shopify dashboard "USD 0.00" hardcoded empty-state currency), F7 (raw "CHARGEBACK_EXPENSE" label in Account Mappings), F14 (no "COD out for delivery" exposure tile), F17 (System Health clearing-balance sign flipped).
