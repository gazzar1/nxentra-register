...
- **F3 (P2, needs design decision):** App-Store-install mid-onboarding dead-ends at "Module Not Enabled" — embedded login should route onboarding-incomplete OWNERs to the wizard's Shopify step (mirror the OAuth-callback behavior in `shopify_connector/views.py:287-291`); also fix the "contact your administrator" copy shown to the OWNER (same copy on `/banking/exceptions` guard).
...
- **P4 batch:** F2 (single-company login shows company chooser), F5 (legacy payout page MM/DD native date inputs), F6 (Shopify dashboard "USD 0.00" hardcoded empty-state currency), F7 (raw "CHARGEBACK_EXPENSE" label in Account Mappings), F11 (Shopify dashboard gross-only revenue tile + CN reason "Goods Returned" on money-only refunds), F14 (no "COD out for delivery" exposure tile), F17 (System Health clearing-balance sign flipped), F1b (dashboard Cash Position counts 11500 clearing as cash).
