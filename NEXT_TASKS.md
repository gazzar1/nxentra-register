...
- **A50 + A51 + A52: tier-1** — must fix **before** continuing App Store submission via the proper data path (or before next real merchant install). A50 + A51 are quick (~45min combined). A52 is the unknown (~0.5-1d depending on root cause). **Tier-1 effort: ~30min + ~15min + ~0.5-1d ≈ 1 day.** Workaround (manual demo data in Nxentra) unblocks the App Store submission itself without fixing these.
...
- **App Store listing** (Built for Shopify checklist, public discoverability): deferred until 5-10 happy paying merchants and product polish reaches submission-grade. Months out, not weeks. Distinct from public OAuth distribution (already enabled, currently submitted via `nxentra-sync-4` deploy).

---

## What to do right now, today

**🔴 2026-05-16 PRIORITY — fix A50/A51/A52 before resuming App Store submission demo path.** Three shopify_connector bugs surfaced 2026-05-15 ~02:30 EEST during reviewer-store setup. A50 + A51 are quick (~45min combined: clamp date range, remove broken button). A52 is the unknown (~0.5-1d): instrument `sync_store_orders` task to find why it returns 0 orders despite 6 existing on `nxentra-reviewer-store`. After fix: re-run import on Shopify_R company, capture 3 screenshots + record screencast (3-8 min unlisted YouTube) + paste reviewer credentials into App Store listing form + Submit for review. Alternative ship-now path if A52 takes too long: create demo data natively in Nxentra (Path 3), submit, fix A52 post-submission.
