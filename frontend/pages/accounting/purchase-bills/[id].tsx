...
export default function PurchaseBillDetailPage() {
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      toast({
        title: "Bill posted",
        description: `Bill ${bill.bill_number} has been posted to the general ledger.`,
      });
    } catch (error: any) {
      toast({
        title: "Error",
  ...
  const handleVoid = async () => {
    ...
    try {
      await voidBill.mutateAsync({ id: bill.id });
      toast({
        title: "Bill voided",
        description: `Bill ${bill.bill_number} has been voided with a reversing entry.`,
      });
    } catch (error: any) {
      toast({
        title: "Error",
        ...
        <PageHeader
          title={
            <div className="flex items-center gap-3">
              ...
              <Badge className={cn("text-sm", BILL_STATUS_COLORS[bill.status])}>
                {BILL_STATUS_LABELS[bill.status]}
              </Badge>
            </div>
          }
          subtitle={`Purchase bill from ${bill.vendor_name}`}
          actions={
            <div className="flex gap-2">
              ...
              <Button
                variant="outline"
                onClick={() => window.open(`/accounting/purchase-bills/${bill.id}/print`, '_blank')}
              >
              ...
              {bill.status === "DRAFT" && (
                <>
                  <Link href={`/accounting/purchase-bills/${bill.id}/edit`}>
                    <Button variant="outline">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <p className="font-medium">{bill.vendor_bill_reference || "—"}</p>
                </div>
                <div>
                ...
                {bill.posted_at && (
                  <div>
                    ...
                    <p className="font-medium">{formatDate(bill.posted_at)}</p>
                  </div>
                )}
              </div>
              ...
              {bill.posted_journal_entry && (
                <div className="mt-4 p-3 bg-muted rounded-lg">
                  <div className="flex items-center gap-2">
                    ...
                    <Link
                      href={`/accounting/journal-entries/${bill.posted_journal_entry}`}
                      className="text-sm font-mono text-primary hover:underline"
                    >
                      {bill.posted_journal_entry_number || `#${bill.posted_journal_entry}`}
                    </Link>
                  </div>
                </div>
              )}
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-sm text-muted-foreground font-mono">{bill.vendor_code}</p>
            </CardContent>
          </Card>
        </div>
              ...
              <div className="w-72 space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-mono text-red-600">
                    -{parseFloat(bill.total_discount).toLocaleString(undefined, {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}
                  </span>
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-mono">
                    {parseFloat(bill.total_tax).toLocaleString(undefined, {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}
                  </span>
      ...
      <AlertDialog open={postDialog} onOpenChange={setPostDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to post bill "{bill.bill_number}"? This will create a journal entry
      ...
      <AlertDialog open={voidDialog} onOpenChange={setVoidDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to void bill "{bill.bill_number}"? This will create a reversing
