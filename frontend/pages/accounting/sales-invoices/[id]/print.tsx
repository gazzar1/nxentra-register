...
import { useSalesInvoice } from "@/queries/useSales";
import { INVOICE_STATUS_LABELS } from "@/types/sales";
      ...
      <div className="print-container p-8 bg-white min-h-screen">
        ...
        <div className="flex justify-between items-start mb-8 border-b pb-4">
          <div>
            ...
            <p className="text-lg font-mono mt-1">{invoice.invoice_number}</p>
          </div>
          <div className="text-right">
            <p className="text-sm text-gray-600">Status</p>
            <p className="font-semibold">{INVOICE_STATUS_LABELS[invoice.status]}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-8 mb-8">
          <div>
            ...
            <p className="text-sm text-gray-600 font-mono">{invoice.customer_code}</p>
          </div>
          <div className="text-right">
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              <span className="font-medium">{invoice.posting_profile_code}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-end">
          <div className="w-72 text-sm">
            ...
            <div className="flex justify-between py-1">
              ...
              <span className="font-mono text-red-600">-{formatNumber(invoice.total_discount)}</span>
            </div>
            <div className="flex justify-between py-1">
              <span className="text-gray-600">Total Tax</span>
              <span className="font-mono">{formatNumber(invoice.total_tax)}</span>
            </div>
        ...
        {invoice.posted_journal_entry && (
          <div className="mt-8 pt-4 border-t">
            <p className="text-sm text-gray-600">
              Journal Entry: <span className="font-mono">{invoice.posted_journal_entry_number || `#${invoice.posted_journal_entry}`}</span>
            </p>
            {invoice.posted_at && (
              <p className="text-sm text-gray-600">Posted: {formatDate(invoice.posted_at)}</p>
            )}
          </div>
        )}
