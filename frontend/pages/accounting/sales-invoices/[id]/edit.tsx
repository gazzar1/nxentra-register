...
export default function EditSalesInvoicePage() {
  ...
  useEffect(() => {
    if (invoice) {
      reset({
        ...
        posting_profile_id: invoice.posting_profile?.toString() || "",
        notes: invoice.notes || "",
  ...
  const onSubmit = async (data: InvoiceFormData) => {
    ...
    try {
      const payload: SalesInvoiceUpdatePayload = {
      ...
      <AppLayout>
        <div className="text-center py-12">
          ...
          <Link href={`/accounting/sales-invoices/${invoice.id}`}>
            <Button variant="link">Back to invoice</Button>
          </Link>
        </div>
      </AppLayout>
    );
