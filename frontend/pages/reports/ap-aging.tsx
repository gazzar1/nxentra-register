...
import { useAPAging } from "@/queries/useReports";
...
function flattenBuckets(buckets: Record<string, AgingBucketEntry[]>): FlatAgingRow[] {
  ...
  for (const [bucket, entries] of Object.entries(buckets)) {
    for (const entry of entries) {
      const code = entry.vendor_code || "";
      const name = entry.vendor_name || "";
      ...
      <div className="space-y-6 print:space-y-0">
        <div className="no-print">
          <PageHeader
            ...
            subtitle="Accounts Payable aging by vendor"
            actions={
          ...
          <CardContent className="pt-6 print:p-0">
            {isLoading ? (
              ...
              <EmptyState
                ...
                description="All vendor balances are settled."
              />
            ) : (
              <>
                <div className="text-center mb-6 print:mb-8">
                  <h2 className="text-xl font-bold">{company?.name}</h2>
                  <h3 className="text-lg mt-2">Accounts Payable Aging Report</h3>
                      ...
                      {filteredRows.map((row) => (
                        <TableRow key={row.code}>
                          <TableCell className="font-mono ltr-code">
                            <Link
                              href={`/reports/vendor-statement?code=${row.code}`}
